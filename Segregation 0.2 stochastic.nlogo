...
turtles-own [
  ...
  %-similar-wanted ;; % similar neighbors wanted  ********  Now a turtle variable!!
]
  ...
  ask patches [
    if random 100 < density [   ;; set the occupancy density
      sprout 1 [
        ...
        set %-similar-wanted random-normal mean-wanted sd-wanted   ;; Now a turtle variable
            if %-similar-wanted > 100 [set %-similar-wanted 100]   ;; Error traps!!
            if %-similar-wanted < 0 [set %-similar-wanted 0]
      ]
    ]
  ]
  update-variables
...
to find-new-spot
  rt random-float 360
  fd random-float max-move    ;;*******Now not hard-coded but a global set on the interface
...
to update-turtles
  ask turtles [
    ifelse (geography = "Moore")
      [set similar-nearby count (turtles-on neighbors)  with [ color = [ color ] of myself ]]
      [set similar-nearby
           count (other turtles with [distance myself < nabe and color = [color] of myself])]
    ifelse (geography = "Moore")
...
to random-migration                                         ;;***all turtles have a small probability of random moving
  let n-movers int (count turtles * migration-rate / 100)   ;;migration rate set on the interface
  ask n-of (n-movers) turtles [find-new-spot]
end
...
@#$#@#$#@
<experiments>
  <experiment name="Experiment 1 max move" repetitions="1000" runMetricsEveryStep="false">
    <setup>setup
set max-move 1 + random 20</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>percent-similar</metric>
    <metric>max-move</metric>
    ...
    <enumeratedValueSet variable="mean-wanted">
      <value value="30"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="sd-wanted">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="geography">
      <value value="&quot;Moore&quot;"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="Experiment 2 sd % wanted" repetitions="1000" runMetricsEveryStep="false">
    <setup>setup
set sd-wanted 0 + random 20</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>percent-similar</metric>
    <metric>sd-wanted</metric>
    <enumeratedValueSet variable="density">
      <value value="95"/>
    </enumeratedValueSet>
  ...
  <experiment name="Experiment 3 mean % wanted" repetitions="1000" runMetricsEveryStep="false">
    <setup>setup
set mean-wanted 20 + random 40</setup>
    <go>go</go>
    <timeLimit steps="1000"/>
    <metric>percent-similar</metric>
    <metric>mean-wanted</metric>
    <enumeratedValueSet variable="density">
      <value value="95"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="max-move">
  ...
  <experiment name="Experiment 4 stochastic" repetitions="8" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
